...
export default function LiveMatchesPage() {
  ...
  async function loadForDate(date: string) {
    ...
    const { data: fixtures } = await supabase
      .from("fixtures")
      .select("id, league_code, round, match_time, home_team_id, away_team_id")
      .eq("match_date", date)
      .order("match_time");
    ...
    const teamIds = Array.from(
      new Set(
        fixtures.flatMap((f) => [
          Number(f.home_team_id),
          Number(f.away_team_id),
        ])
      )
    );
    ...
    (teams || []).forEach((t) => (teamMap[Number(t.id)] = t.name));
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-5">
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-bold text-[#0A5E2A]">Live unos — utakmice</h1>
      ...
      <div className="space-y-3">
        {matches.map((m) => (
          <Link
            ...
            className="block bg-white p-4 rounded-xl border shadow hover:bg-[#f7f1e6] transition"
          >
            <div className="flex justify-between items-center">
              <div>
